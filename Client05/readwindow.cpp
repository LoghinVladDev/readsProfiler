...
void ReadWindow::addToReadWindow(const char *fileAbsPath)
{
    ...
    if(!strcmp(extension, "pdf"))
    {
        ...
        /*
        Poppler::Document* document = Poppler::Document::load(QString(fileAbsPath));
        if(!document || document->isLocked())
        {
            errorBox("Could not open document");
            delete document;
            return;
        }

        QWidget* docWidget = new DocumentWidget( document,this->window );
        this->browserLayout->addWidget(docWidget);

        QPixmap * pixmap = new QPixmap();

        QTableWidget* imageTable = new QTableWidget();
        imageTable->setColumnCount(1);
        imageTable->setColumnWidth(0, this->window->width()-10);
        for(int i = 0 ; i < document->numPages(); i++)
        {
            //QImage* img = new QImage();
            //img = document->page(i)->renderToImage();//this->window->width()-30, this->window->height()-40);

            QTableWidgetItem *thumbnail = new QTableWidgetItem();
            thumbnail->setData(Qt::DecorationRole,  QPixmap::fromImage(document->page(i)->renderToImage(100,100, 10,10,80,80)));
            //thumbnail->setIcon(QIcon(QPixmap::fromImage(document->page(i)->renderToImage()).scaled(100,100)));
            imageTable->setItem(i,0, thumbnail);
            printf("%d ce plm\n", i);
            fflush(stdout);

            //delete img;
        }
        this->browserLayout->addWidget(imageTable);*/
    }
}
